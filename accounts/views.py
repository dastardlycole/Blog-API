...
from accounts.models import Post, Like
from .serializers import ResendOTPSerializer, VerifyPasswordOTPSerializer, UserSerializer, LoginSerializer, VerifyOTPSerializer,PostSerializer, ForgotSerializer , LikeSerializer, ResetSerializer
...
def create_post(request):
    ...
    if request.method == "POST":
        ...
        if serializer.is_valid(): 
            if user in User.objects.filter(is_active = True): 
                message = serializer.validated_data["message"]

                post = Post.objects.create(user=user,message=message)
                new_serializer = PostSerializer(post)
                ...
                data = {
                    'message' : 'Log into an active account to perform this action',
                    'error'  : serializer.errors
                }
                return Response(data, status=status.HTTP_400_BAD_REQUEST)     
        else:
    ...
    if request.method == 'GET':
        ...
        for i in serializer.data:
            ...
            user_dict = model_to_dict(user, ['id', 
                                'first_name',
                                'last_name',])
            user_dict['post_id'] = i['id']                    
            user_dict['message'] = i['message']                    
            user_dict['date_created'] = i['date_created']                    
...
def post_detail(request, post_id):
    ...
    if request.method == "GET":
        ...
        if post.user != request.user:
            raise PermissionDenied(detail={"message":"You do not have the permission to edit this item."})
...
def create_like(request):
    ...
    if request.method == "POST":
        ...
        if 'user' in serializer.initial_data.keys():
            serializer.initial_data.pop('user')
        
        if serializer.is_valid(): 
            if user in User.objects.filter(is_active = True): 
                check = Like.objects.filter(user = user, like = True, post = serializer.validated_data['post'] )
                ...
                if check.exists():
                    ...
                    like = Like.objects.create(user=user,like=like, post=post)
                    new_serializer = LikeSerializer(like)
...
def view_likes(request):
    ...
    if request.method == 'GET':
        # Get all the users in the database
        all_likes = Like.objects.filter(user = user, like = True)
...
@authentication_classes([BasicAuthentication])
@permission_classes([IsAuthenticated])
def view_likers(request, post_id):
    user = request.user
    if request.method == 'GET':
        # Get all the users in the database
        all_likes = Like.objects.filter(post=post_id, like = True)
